...
function upkeepify_provider_response_form_shortcode($atts) {
    ...
    if ($query->have_posts()) {
        ...
        if ( empty( $decision ) ) {
            ...
            echo '<p class="upkeepify-field"><label for="upkeepify-duration">' . esc_html__( 'Estimated duration (optional)', 'upkeepify' ) . '</label><br>';
            echo '<input type="number" name="estimate_duration" id="upkeepify-duration" class="upkeepify-input" min="1" step="1" placeholder="' . esc_attr__( 'Number of days', 'upkeepify' ) . '"> ';
            echo '<small>' . esc_html__( 'days', 'upkeepify' ) . '</small></p>';

            echo '<p class="upkeepify-field"><label for="upkeepify-start-date">' . esc_html__( 'Estimated possible start date (optional)', 'upkeepify' ) . '</label><br>';
            echo '<input type="date" name="estimated_start_date" id="upkeepify-start-date" class="upkeepify-input" min="' . esc_attr( gmdate( 'Y-m-d' ) ) . '"></p>';

            echo '<p class="upkeepify-field"><label for="upkeepify-end-date">' . esc_html__( 'Estimated end date (optional)', 'upkeepify' ) . '</label><br>';
            echo '<input type="date" name="estimated_end_date" id="upkeepify-end-date" class="upkeepify-input" min="' . esc_attr( gmdate( 'Y-m-d' ) ) . '"></p>';
...
(function () {
    ...
    updateVisibility();
            ...
            echo '<p class="upkeepify-field"><label for="upkeepify-quote-duration">' . esc_html__( 'Estimated duration (optional)', 'upkeepify' ) . '</label><br>';
            echo '<input type="number" name="estimate_duration" id="upkeepify-quote-duration" class="upkeepify-input" min="1" step="1" placeholder="' . esc_attr__( 'Number of days', 'upkeepify' ) . '"> ';
            echo '<small>' . esc_html__( 'days', 'upkeepify' ) . '</small></p>';

            echo '<p class="upkeepify-field"><label for="upkeepify-quote-start-date">' . esc_html__( 'Estimated possible start date (optional)', 'upkeepify' ) . '</label><br>';
            echo '<input type="date" name="estimated_start_date" id="upkeepify-quote-start-date" class="upkeepify-input" min="' . esc_attr( gmdate( 'Y-m-d' ) ) . '"></p>';

            echo '<p class="upkeepify-field"><label for="upkeepify-quote-end-date">' . esc_html__( 'Estimated end date (optional)', 'upkeepify' ) . '</label><br>';
            echo '<input type="date" name="estimated_end_date" id="upkeepify-quote-end-date" class="upkeepify-input" min="' . esc_attr( gmdate( 'Y-m-d' ) ) . '"></p>';
...
function upkeepify_admin_post_provider_response_submit() {
    ...
    // Optional estimate duration (int days).
    $duration = isset( $_POST['estimate_duration'] ) ? absint( wp_unslash( $_POST['estimate_duration'] ) ) : 0;
    if ( $duration >= 1 ) {
        update_post_meta( $response_id, UPKEEPIFY_META_KEY_RESPONSE_ESTIMATE_DURATION, $duration );
    }

    // Optional estimated start date (Y-m-d).
    $start_date = isset( $_POST['estimated_start_date'] ) ? sanitize_text_field( wp_unslash( $_POST['estimated_start_date'] ) ) : '';
    if ( $start_date && preg_match( '/^\d{4}-\d{2}-\d{2}$/', $start_date ) ) {
        update_post_meta( $response_id, UPKEEPIFY_META_KEY_RESPONSE_ESTIMATED_START_DATE, $start_date );
    }

    // Optional estimated end date (Y-m-d).
    $end_date = isset( $_POST['estimated_end_date'] ) ? sanitize_text_field( wp_unslash( $_POST['estimated_end_date'] ) ) : '';
    if ( $end_date && preg_match( '/^\d{4}-\d{2}-\d{2}$/', $end_date ) ) {
        update_post_meta( $response_id, UPKEEPIFY_META_KEY_RESPONSE_ESTIMATED_END_DATE, $end_date );
    }
